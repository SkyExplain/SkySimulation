...
#-------------------------------------------------------------------------------------
omega_cdms = np.linspace(0.1, 0.15, 2) #Planck: omega_cdm = 0.12011
nss = np.linspace(0.94, 1.02, 2) #Planck: ns = 0.9649
...
for omega_cdm in omega_cdms:
    for ns in nss:
        ...
        save_power_spectrum(f"{output_spct_lcdm}dlstt_lcdm_{flag_lcdm}_NSIDE64.csv", round_ls_Pl_TT, dlstt_noisy_lcdm)
        ...
        output_lcdm = "./simulated_maps/tests_NSIDE64/"
...
omega_cdms = np.linspace(0.1, 0.15, 2)
A_lins = np.linspace(0.1, 0.6, 2)
...
for omega_cdm in omega_cdms:
    for A_lin in A_lins:
        ...
        save_power_spectrum(f"{output_spct_f}dlstt_feature_{flag_feature}_NSIDE64.csv", round_ls_Pl_TT, dlstt_noisy_feature)
        ...
        output_feature = "./simulated_maps/tests_NSIDE64/"
        #Generate and save the temperature map
